...
chipsEl.innerHTML = chips.map((c, i) => ` <button class="filter-chip" onclick="(${c.clear.toString()})()" aria-label="Remove filter: ${c.label}"> ${c.label} &times; </button>`).join(’’);
}

// –– Update mobile filter button badge ––
function updateFilterCount() {
const total = filterState.color.length + filterState.status.length;
...
if (!wrap) return;
wrap.innerHTML = COLORS.map(c => ` <button class="swatch-btn" data-color="${c.value}" style="--swatch-color:${c.hex}" onclick="toggleColorFilter('${c.value}', this)" aria-label="${c.label}" aria-pressed="false" title="${c.label}"> </button>`).join(’’);
}
...
document.getElementById(‘finishing-options’).innerHTML = p.finishing.map(f => ` <button class="finishing-btn ${f === selectedFinishing ? 'selected' : ''}" onclick="selectFinishing('${f}', this)">${f}</button>`).join(’’);
...
thumbs.innerHTML = p.images.map((img, i) => ` <button class="thumb-btn ${i === 0 ? 'active' : ''}" onclick="swapMainImage('${img}', this)" aria-label="View image ${i + 1}"> <img src="${img}" alt="${p.name} view ${i + 1}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=thumb-placeholder style=background:${p.placeholderColor}></div>'"> </button>`).join(’’);
} else {
mainImg.src = ‘’;
mainImg.alt = ‘’;
